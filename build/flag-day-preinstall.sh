...
#          two callers in build/package-{native,cross}-pkg.sh and tests/flag-day-preinstall-test.sh.
#
#          The pkg identifier moved from dev.modernmavericks.clang.* to dev.mavergreen.clang.*, and to
...
#          else the old identity left is already handled: the toolchain prefix did not move (this
#          payload overwrites it), and the old updater .app and its agent are retired by shipyard's
#          shared updater/agent-load.in, which the postinstall stage_updater.sh renders already runs.
set -eu
